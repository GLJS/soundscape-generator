#!/usr/bin/env python3
"""
Hybrid streaming processor using SGLang for caption generation.
Processes audio in batches while streaming results to tar files.
"""
...
# Import SGLang implementation
from utils_sglang_clean import AudioTextDataset, collate_batch, GemmaProcessor
...
class HybridStreamingProcessor:
    ...
    Uses SGLang for caption generation.
    """
    ...
    def __init__(self,
        ...
        """Initialize hybrid streaming processor with SGLang."""
        ...
        logger.info(f"Initialized with batch_size={batch_size}, workers={num_workers}")
    ...
    def tar_writer_thread(self, prefix: str = "data", split: Optional[str] = None):
        ...
        logger.info(f"Tar writer finished. Written: {self.stats['written']}")
    ...
    def process_dataset(self, audio_files: List[Tuple[Path, str, Dict]], 
        ...
        """Process dataset using hybrid streaming approach."""
        ...
        try:
            ...
            for batch in iterator:
                ...
                # Generate captions
                batch, generated_texts = self.gemma_processor.generate_batch(batch)
                if not generated_texts:
                    continue
                
                # Get audio outputs
                audio_outputs = batch.get('audio_outputs', [])
                
                # Submit results to tar queue
                for audio_48khz, caption, metadata in zip(
                    audio_outputs, generated_texts, batch['metadata']
                ):
                    if audio_48khz is not None:
        ...
        logger.info(f"Complete. Processed: {self.stats['processed']}, "
