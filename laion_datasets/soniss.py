#!/usr/bin/env python3
"""
Convert Soniss dataset to WebDataset tar format.

Audio location: /scratch-shared/gwijngaard/laion/soniss/
...
class SonissProcessor(DatasetProcessor):
    """Processor for Soniss dataset."""
    ...
    def load_metadata(self) -> pd.DataFrame:
        """Load metadata - returns empty DataFrame as Soniss has no CSV."""
        print(f"Loading audio files from {self.audio_dir}")
        # No metadata CSV for Soniss dataset
        return pd.DataFrame()
    ...
    def match_audio_to_text(self, metadata_df: pd.DataFrame) -> List[Tuple[Path, str, Dict]]:
        ...
        if len(audio_files) > 0:
            ...
            for audio_path in tqdm(audio_files, desc="Collecting audio files"):
                try:
                    ...
                    parent_dir = audio_path.parent.name
                    
                    # Process directory name: if contains " - ", take the part after last " - "
                    if " - " in parent_dir:
                    ...
                    combined_text = f"{dir_name} {filename}"
                    
                    metadata = {
                        'split': 'train',  # All files go to train split
                        'original_filename': filename,
                        'directory_name': dir_name,
                        'full_directory': parent_dir,
            ...
            # Process with Gemma using directory + filename as context
            gemma_results = process_dataset_with_gemma(
                audio_paths=audio_paths,
                texts=texts,  # Directory + filename as context
        ...
        with open("matched_soniss.json", "w") as f:
            json.dump(matched_serializable, f)
        print(f"Saved {len(matched)} audio-text pairs to matched_soniss.json")
        
        return matched


def main():
    parser = argparse.ArgumentParser(description="Convert Soniss dataset to tar format")
    parser.add_argument("--audio-dir", type=str, 
                       default="/scratch-shared/gwijngaard/laion/soniss",
                       help="Path to Soniss audio directory")
    parser.add_argument("--metadata", type=str,
                       default="/scratch-shared/gwijngaard/laion/soniss",
                       help="Path to metadata (not used for Soniss)")
    parser.add_argument("--output-dir", type=str,
                       default="/scratch-shared/gwijngaard/tar/soniss",
