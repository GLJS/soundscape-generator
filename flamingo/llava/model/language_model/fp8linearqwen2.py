...
# Try to import additional functions that might not be available in all versions
try:
    from transformers.models.qwen2.modeling_qwen2 import apply_rotary_pos_emb, repeat_kv, rotate_half
except ImportError:
    # Import from transformers.modeling_attn_mask_utils or define fallbacks
    from transformers.modeling_attn_mask_utils import _prepare_4d_causal_attention_mask
    # These functions might be in different locations in newer versions
    apply_rotary_pos_emb = None
    repeat_kv = None
    rotate_half = None

# Try to import Qwen2FlashAttention2 and Qwen2SdpaAttention, fallback to Qwen2Attention if not available
try:
...
class FP8LinearQwen2Model(FP8LinearQwen2PreTrainedModel):
    ...
    # Note: _update_causal_mask might not exist in newer transformers versions
    if hasattr(Qwen2Model, '_update_causal_mask'):
