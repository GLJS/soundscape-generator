#!/bin/bash
# Run AudioDataFull with SGLang server
...
if [[ $(hostname) == gcn* ]]; then
    ...
    echo "srun --partition=gpu_h100 --time=2:00:00 --gpus-per-node=1 --exclude=gcn131,gcn138 bash run_sglang_audiodatafull.sh"
    exit 1
fi
...
# Run processor
echo "Running AudioDataFull processor..."
python laion_datasets/audiodatafull_sglang.py \
    --audio-dir /scratch-shared/gwijngaard/laion/AudioDataFull/ \
